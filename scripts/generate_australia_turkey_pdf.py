...
# -*- coding: utf-8 -*-
"""
Google Material Design 风格 PDF 报告生成器
澳大利亚 vs 土耳其 - 2026世界杯六维预测报告
"""
...
OUTPUT_FILE = os.path.join(DESKTOP, "澳大利亚vs土耳其_六维预测报告.pdf")
...
def create_styles():
    ...
    styles.add(ParagraphStyle(
        name='GoogleHighlight',
        fontName='SimHei',
        fontSize=12,
        textColor=GOOGLE_GREEN,
        spaceAfter=6,
        leading=15
    ))
...
def create_prediction_table(title, headers, data, highlight_col=None):
    """创建预测表格"""
...
def generate_report():
    ...
    story.append(create_prediction_table(
        "1X2",
        ['项目', '澳大利亚', '平局', '土耳其'],
        [
            ['赔率', '5.20', '3.67', '1.70'],
    ...
    story.append(Paragraph(
        "免责声明: 本报告为研究模型输出，不构成投资建议。足球比赛具有高度不确定性，请理性看待。",
        styles['GoogleFooter']
    ))
    story.append(Paragraph(
        f"报告生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Football Quant OS v4.3.2",
        styles['GoogleFooter']
    ))
