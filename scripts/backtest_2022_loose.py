...
class LooseHybridRunner:
    ...
    def run_hybrid(self, match, odds):
        ...
        if not market_data:
            ...
            self.results["matches"].append({
                "match_id": match["match_id"], "match": f"{match['home']} {match['home_score']}-{match['away_score']} {match['away']}",
                "outcome": actual, "action": "ABANDON", "bet_dir": None, "bet_amount": 0,
                "motivation": 0, "pd": 0, "model": model, "market": {"home": 0.33, "draw": 0.33, "away": 0.33},
            })
            return
        ...
        # LOOSE thresholds: PD >= 0.015, edge >= 0.03
        if abs(max_pd) >= 0.015 and model[value_dir] > market_prob[value_dir] + 0.03:
            action = "EXECUTE"
            bet_dir = value_dir
    ...
    def run(self):
        ...
        print("=" * 60)
        print("2022 World Cup - Loose Hybrid (A+B) Backtest")
        print("LOOSE: PD >= 0.015, Edge >= 0.03, Kelly 0.35, Max 2%")
        print("=" * 60)
    ...
    def print_report(self):
        ...
        print("\n" + "=" * 60)
        print("BACKTEST RESULTS - Loose Hybrid (A+B)")
        print("=" * 60)
        ...
        hybrid = r["hybrid"]
        print(f"\n[2] LOOSE HYBRID (A + B + Loose Thresholds)")
        ...
        print(f"  Loose Hybrid vs Elo: EUR {diff:+.0f} ({diff/self.bankroll_start*100:+.1f}%)")
        print(f"  Capital Efficiency: Loose {hybrid['bets']/hybrid['total']*100:.1f}% vs Elo {elo['bets']/elo['total']*100:.1f}%")
        
        print(f"\n[4] TOP EXECUTED BETS")
        executes = [m for m in r["matches"] if m["action"] == "EXECUTE"]
        sorted_exec = sorted(executes, key=lambda x: x["bet_amount"], reverse=True)[:12]
        ...
        with open(os.path.join(os.path.dirname(__file__), "data", "backtest_2022_loose.json"), 'w') as f:
            json.dump(r, f, indent=2)
        print("Results saved to backtest_2022_loose.json")

if __name__ == "__main__":
    runner = LooseHybridRunner(bankroll=10000)
    runner.run()
