...
styles.add(ParagraphStyle(name='RWarning', fontName='YaHei-Bold', fontSize=12, leading=18, textColor=ACCENT_RED, alignment=TA_CENTER, spaceAfter=10*mm, backColor=WARNING_BG))
...
def generate():
    ...
    output = os.path.join(desktop, 'Naga_Quant_Report_Qatar_Swiss_REVISED.pdf')
    ...
    story.append(Paragraph("2026年6月13日 18:13 | 数据来源：500.com / OddsPortal / Betfair Exchange", styles['RMeta']))
    
    # WARNING BOX
    story.append(Paragraph(
        "【修正说明】2026世界杯东道主为美国、加拿大、墨西哥。卡塔尔不是东道主，比赛场地为美国加州Levi's Stadium（中立场地）。"
        "此前报告错误假设卡塔尔拥有主场优势，现已修正。此修正显著下调了卡塔尔预期进球（xG）和胜率评估。",
        styles['RWarning']))
    ...
    story.append(Paragraph(
        "修正后的模型显示：卡塔尔在无主场优势的中立场地作战，真实实力为FIFA第50名，近期4场比赛仅打入1球。"
        "瑞士排名第15，近期5场进9球，但进攻表现不稳定（0-0挪威、1-1澳大利亚、1-1科索沃）。"
        "市场因89%资金押注瑞士而将客队赔率压缩至1.24，隐含概率高达80%。"
        "模型评估瑞士真实胜率约62%，存在约18%的过度追捧。"
        "卡塔尔方向仍有价值（市场给6%，模型给24%），但属高风险投机。让球+2.0为最安全价值选项。",
        styles['RBody']))
    ...
    key_data = [
        ...
        ['卡塔尔xG', '1.1', '0.25', '下调77%'],
        ['总进球预期', '2.6', '2.0', '下调23%'],
        ['卡塔尔胜率', '26.4%', '23.8%', '小幅下调'],
        ['1X2 Edge', '+20.2%', '+17.5%', '仍高但风险增加'],
    ...
    # Market vs Model
    story.append(Paragraph("1. 市场隐含概率 vs 修正后模型概率", styles['RH3']))
    prob_data = [
        ['市场', '主胜（卡塔尔）', '平局', '客胜（瑞士）', 'Edge偏差'],
        ['市场隐含', '6.2%', '13.5%', '80.3%', '-'],
        ['修正模型', '23.8%', '13.9%', '62.4%', '-'],
        ['差异', '+17.5%', '+0.4%', '-17.9%', '卡塔尔被低估'],
    ]
    ...
    prob_table.setStyle(TableStyle([
        ...
        ('BACKGROUND', (0,0), (-1,0), TABLE_HEADER), ('FONTNAME', (0,0), (-1,0), 'YaHei-Bold'),
        ('TEXTCOLOR', (0,0), (-1,0), BLUE),
        ('BACKGROUND', (0,3), (-1,3), WARNING_BG), ('FONTNAME', (0,3), (-1,3), 'YaHei-Bold'),
    ]))
    story.append(prob_table)
    story.append(Spacer(1, 5*mm))
    
    # Multi-Market
    story.append(Paragraph("2. 多市场预测与推荐（修正版）", styles['RH3']))
    
    story.append(Paragraph("【胜平负 1X2】", styles['RH4']))
    story.append(Paragraph(
        f"<b>预测：</b>卡塔尔主胜 | <b>最佳赔率：</b>16.00（Betfair Exchange）| <b>Edge：</b><font color='#006600'>+17.5%</font> | <b>推荐：极小注（TINY）</b><br/>"
        f"<b>风险警告：</b>虽然Edge高达+17.5%，但卡塔尔真实实力弱（FIFA#50，近期4场1进球，无主场优势）。"
        f"此Edge完全来自市场过度追捧瑞士（89%资金），而非卡塔尔真实竞争力。"
    ...
    story.append(Paragraph(
        f"<b>预测：</b>卡塔尔+2.0 | <b>最佳赔率：</b>1.67（bet365）| <b>Edge：</b><font color='#006600'>~+6.0%</font> | <b>推荐：小注（SMALL）</b><br/>"
        f"逻辑：瑞士进攻表现不稳定（近5场对弱队仅1场大比分：4-1约旦；其余为0-0、1-1、1-1、3-4）。"
        f"+2.0提供3球缓冲，即使瑞士2-0或3-1获胜，卡塔尔仍覆盖。"
        f"此为组合中最安全的仓位，推荐投入1.5%本金。",
        styles['RBody']))
    
    story.append(Paragraph("【半全场 HT/FT】", styles['RH4']))
    story.append(Paragraph(
        f"<b>预测：</b>半场平/全场客胜 | <b>赔率：</b>~4.50 | <b>Edge：</b>~+1.5% | <b>推荐：回避</b><br/>"
        f"逻辑：瑞士上半场常试探，卡塔尔上半场可死守。但Edge仅1.5%，低于2%阈值，无统计显著性。建议不投入。",
        styles['RBody']))
    ...
    story.append(Paragraph(
        ...
        f"0:2次之（20.9%）。卡塔尔进球概率极低（1:1仅5.8%）。",
        styles['RSmall']))
    
    story.append(Paragraph("【大小球 Over/Under】", styles['RH4']))
    story.append(Paragraph(
        f"<b>预测：</b>Over 2.5 @ 1.61 | <b>模型概率：</b>{p_over25:.1%} | <b>市场概率：</b>{m_over25_norm:.1%} | <b>Edge：</b><font color='#CC0000'>{edge_ou:+.1%}</font> | <b>推荐：回避</b><br/>"
        f"逻辑：修正后预期总进球仅{total_xg:.1f}球。市场隐含Over 2.5概率为{m_over25_norm:.1%}，"
        f"高于模型{p_over25:.1%}。Over 2.5为负期望值。瑞士进攻不稳定，卡塔尔几乎不进球，"
        f"最可能为1-2球区间。建议不投入。",
        styles['RBody']))
    
    story.append(Paragraph("【总进球数】", styles['RH4']))
    story.append(Paragraph(
        f"预期：{total_xg:.1f}球 | 分布：0-1球（{p_0+p_1:.1%}）| 2-3球（{p_2+p_3:.1%}）| 4+球（{p_4:.1%}）<br/>"
    ...
    # Portfolio
    story.append(Paragraph("3. 资金配置与风险管理（保守策略）", styles['RH3']))
    story.append(Paragraph(
        f"<b>假设本金：</b>10,000单位 | <b>单次最大暴露：</b>3%（300单位）| <b>Kelly Fractional：</b>1/4",
        styles['RBody']))
    story.append(Spacer(1, 2*mm))
    
    portfolio_data = [
        ['优先级', '市场', '预测', '赔率', '投入', 'Kelly', '风险等级'],
        ['1', '让球 AH', '卡塔尔+2.0', '1.67', '150单位', '1.5%', '低风险（推荐）'],
        ['2', '比分', '0:1', '~6.00', '80单位', '0.8%', '中风险（小注）'],
        ['3', '比分', '0:2', '~8.00', '70单位', '0.7%', '中风险（小注）'],
        ['4', '1X2', '卡塔尔主胜', '16.00', '50单位', '0.5%', '极高风险（极小）'],
    ]
    portfolio_table = Table(portfolio_data, colWidths=[18*mm, 28*mm, 35*mm, 22*mm, 22*mm, 20*mm, 35*mm])
    ...
    risks = [
        "模型修正风险：此前错误假设卡塔尔为东道主，虽已修正，但模型可能仍存在其他偏差",
        ...
        "数据时效：报告基于2026-06-13 18:13数据，赛前赔率可能漂移，建议赛前30分钟复核",
    ]
    for risk in risks:
    ...
    story.append(Paragraph(
        ...
        "REVISED REPORT | Correction: Qatar is NOT host | 2026-06-13 18:13 GMT+8",
        styles['RDisclaimer']))
